...
old_teams_full <- teams %>% 
  ...
  select(year_id, franch_id,hbp, e, sb, l, r, ab, h, hr, bb, so,g, attendance,era) %>% 
...
ggplot(old_teams_full, aes(year_id, att_per_game, color = franch_id)) +
  geom_line() +
